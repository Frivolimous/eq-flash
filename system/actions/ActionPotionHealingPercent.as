package system.actions{
	...
	public class ActionPotionHealingPercent extends ActionBase{
		var flatHealing:Number=0;
		
		public function ActionPotionHealingPercent(_label:String=null,_level:int=0,_damage:Number=0,_effects:Vector.<EffectBase>=null,_type:int=DamageModel.HOLY,_flat:Number=0){
			if (_label!=null){
				baseStats();
				label=_label;
				level=_level;
		...
		override public function modify(_v:SpriteModel,_random:Boolean=true):ActionBase{
			if (_random){
				var m:ActionPotionHealingPercent=(clone(getBoost(_v)) as ActionPotionHealingPercent);
			}else{
				m=(clone() as ActionPotionHealingPercent);
			}
			...
			if (damage<1){
				...
				if (isDrinking()) m.damage*=_v.stats.getValue(StatModel.POTEFF);
			}
			if (m.flatHealing>0){
				if (isConsumable()) m.flatHealing*=_v.stats.getValue(StatModel.ITEMEFF);
				switch (m.type){
					case DamageModel.HOLY: m.flatHealing*=_v.stats.getValue(StatModel.HOLYEFF); break;
					case DamageModel.CHEMICAL: m.flatHealing*=_v.stats.getValue(StatModel.CHEMEFF); break;
					case DamageModel.PHYSICAL: m.flatHealing*=_v.stats.getValue(StatModel.PHYSEFF); break;
					case DamageModel.MAGICAL: m.flatHealing*=_v.stats.getValue(StatModel.MAGICEFF); break;
					case DamageModel.DARK: m.flatHealing*=_v.stats.getValue(StatModel.DARKEFF); break;
				}
				m.flatHealing*=_v.stats.getValue(StatModel.HEALMULT);
				if (isDrinking()) m.flatHealing*=_v.stats.getValue(StatModel.POTEFF);
			}
		...
		override public function postAnim(_o:SpriteModel,_t:SpriteModel){
			dmgModel.setDmg(_t.stats.getValue(StatModel.HEALTH)*damage+flatHealing,type);
			_t.healthHeal(_t.stats.getValue(StatModel.HEALTH)*damage+flatHealing);
		...
		override public function setDefaultPriorities(){
			additionalPriorities=[new ConditionalBase(1,0,[3,4])];
			priorityTier=1;
		...
		override public function getDesc():String{
			...
			m+="\n HEAL: <font color="+StringData.RED+"><b>";
			if (flatHealing>0) m+=String(Math.round(flatHealing))+" +";
			m+=Math.round(damage*100)+"% ";
