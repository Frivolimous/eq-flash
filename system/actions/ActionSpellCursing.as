package system.actions{
	...
	public class ActionSpellCursing extends ActionBase{
		public function ActionSpellCursing(_label:String=null,_level:int=0,_effects:Vector.<EffectBase>=null,_mana:int=0,_tags:Array=null){
			if (_label!=null){
				baseStats();
				label=_label;
				level=_level;
		...
		override public function canUse(_o:SpriteModel,_t:SpriteModel,_distance:String=null):Boolean{
			...
			for (var i:int=0;i<_o.buffList.numBuffs();i+=1){
				switch(_o.buffList.getBuff(i).name){ //limited actions
					case BuffData.AIMING: case BuffData.BERSERK: case BuffData.TAUNT: case BuffData.SILENCED: case BuffData.INITIAL_NO_OFFENSIVE: return false;
				}
			}
		...
		override public function wantUse(_o:SpriteModel,_t:SpriteModel,_distance:String=null,_rate:Boolean=true):Boolean{
			...
			if (effects.length>0 && (effects[0] is EffectBuff)){
				var _buff:BuffBase=_t.buffList.getBuffCalled((effects[0] as EffectBuff).buff.name);
				if (_buff!=null && _buff.stacks>=_buff.maxStacks){
