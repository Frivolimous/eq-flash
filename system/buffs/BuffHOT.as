package system.buffs{
	...
	public class BuffHOT extends BuffBase{
		var mult:int;
		public var healing:Number;
		public var healType:int;
		
		public function BuffHOT(_index:int,_name:String,_level:int,_charges:int,_mult:int=-1,_healing:Number=0,_healType:int=0,_maxStacks:int=1){
			name=_name;
			index=_index;
			level=_level;
			type=BuffBase.BUFF;
		...
		override public function modify(_v:SpriteModel,_moreMult:Number=1):BuffBase{
			var m:BuffHOT=clone() as BuffHOT;
		...
		override public function clone():BuffBase{
			return new BuffHOT(index,name,level,charges,mult,healing,healType,maxStacks);
		}
		
		override public function addStacksFrom(_removed:BuffBase){
			if (_removed.stacks<maxStacks){
				healing+=(_removed as BuffHOT).healing;
			}else{
				healing=(_removed as BuffHOT).healing;
			}
