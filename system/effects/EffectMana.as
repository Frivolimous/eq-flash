package system.effects{
	...
	public class EffectMana extends EffectBase{
		public var heal:Number,
					healType:int;
		
		public function EffectMana(_name:String=null,_heal:Number=0,_hType:int=0,_trigger:int=-1,_userate:Number=1){
			if (_name!=null){
				name=_name;
				level=0;
		...
		override public function modify(_v:SpriteModel,_action:ActionBase=null):EffectBase{
			var m:EffectMana=(clone() as EffectMana);
		...
		override public function clone():EffectBase{
			return new EffectMana(name,heal,healType,trigger,userate);
		}
		...
		override public function getDesc(_tabs:int=1):String{
			...
			m+="MANA: <font color="+StringData.RED+"><b>+"+StringData.reduce(heal)+" "+DamageModel.shortTypeName[healType]+"</b></font>";
		...
		override public function getSpecialDesc(_descType:int=-1):String{
			...
			m+="Recover an amount of your mana.";
			return m;
		}
	}
}