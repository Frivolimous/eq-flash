package items{
	...
	public class ItemData{
		...
		public static function spawnItem(_level:int,_index:int,_charges:int=-1):ItemModel{
			if (_index<=63){
				...
				switch(_index){
					...
					case 149: return new ItemModel(_index,"Blank",_level,"MYSTERY","UNKNOWN",null,2000000);
		...
		public static function spawnPremium(_level:int,_super:Boolean=true):ItemModel{
			if (_super && Math.random()<0.1){
		...
		public static function finishGamblePremium(_item:ItemView){
			_item.update(spawnPremium(0,true));
		}
